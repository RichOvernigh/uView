  ...
  <view :class="'interlayer ' + (c || '')" :style="s">
    <block v-for="(n, i) in nodes" :key="i">
      ...
      >
        ...
        <image
          class="_image"
          :src="lazyLoad && !ctrl[i] ? placeholder : getThumbnail(n.attrs.src)"
          :lazy-load="lazyLoad"
      ...
      >
        ...
        <trees class="_li" c="_li" :nodes="n.children" :lazy-load="lazyLoad" :loading="loading" />
      </view>
      <!--表格-->
      <view
    ...
    methods: {
      ...
      getThumbnail(src) {
        if (src && src.includes('png')) {
          return `${src}?imageView2/0/format/webp/q/30`;
        }
        return src;
      },
      error(e) {
