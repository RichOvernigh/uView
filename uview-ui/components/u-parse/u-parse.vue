...
<template>
  <view>
    ...
    >
      ...
      <!--#endif-->
      <!--#ifndef H5 || MP-360-->
      <trees :nodes="nodes" :lazy-load="lazyLoad" :loading="loadingImg" :preview-img="previewImg" />
      <!--#endif-->
    </view>
    <!--#endif-->
  </view>
...
<script>
  // eslint-disable
  //  eslint-disable max-len import/order
  var search;
        ...
        var selector = uni
          ...
          .select(
            (this._in ? this._in.selector : '#_top') +
              (obj.id
                ? `${d}#${obj.id},${this._in ? this._in.selector : '#_top'}${d}.${obj.id}`
                : ''),
          )
          .boundingClientRect();
        if (this._in)
